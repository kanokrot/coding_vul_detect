...
def hybrid_scanning_system(file_obj, git_url):
    ...
    empty = ("Waiting for input...", "", pd.DataFrame(), "")
    ...
    for fname, content in files_to_scan:
    ...
    <div style="display:flex; gap:12px; margin:12px 0;">
        <div class="summary-card critical">Critical: {critical_count}</div>
        <div class="summary-card high">High: {high_count}</div>
        <div class="summary-card medium">Medium: {medium_count}</div>
        <div class="summary-card low">Low: {low_count + safe_count}</div>
    </div>
    """
...
"""
    ...
    return summary_text, cards_html, df, remediation_text